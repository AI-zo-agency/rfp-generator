...
export function LeadFinderClient({ payload }: { payload: LeadsPayload }) {
  ...
  async function toggle(lead: LeadRow) {
    ...
    void Promise.allSettled([loadBrief(lead.id), runEnrich(lead.id)]);
  }
  ...
  async function runEnrich(id: string) { setEnrichmentStatuses((prev) => ({ ...prev, [id]: "loading" })); try { const response = await fetch(`/api/leads/${id}/enrich`, { method: "POST" }); const enrichment = await response.json(); if (!response.ok) throw new Error(enrichment?.detail ?? "Enrichment failed."); setEnrichments((prev) => ({ ...prev, [id]: enrichment })); setEnrichmentStatuses((prev) => ({ ...prev, [id]: "ready" })); } catch { setEnrichmentStatuses((prev) => ({ ...prev, [id]: "unavailable" })); } }
