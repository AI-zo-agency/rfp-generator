...
def test_the_live_position_nets_positive_despite_the_headline_ratio():
    ...
    out = position(_live())
    assert out["cash_on_hand"] == "$7,742"
    assert out["bills_you_owe_overdue"] == "$26,973"
    assert out["invoices_owed_to_you_overdue"] == "$28,498"
    assert out["net_due_now"] == "$9,268"
    assert round(out["net_due_now_amount"], 2) == 9_267.92


def test_not_yet_due_is_excluded_from_both_sides():
    out = position(_live())
    # $11,671 of bills and $22,746 of invoices are not yet due; neither counts.
    assert out["bills_you_owe_overdue"] != "$38,643"
    assert out["invoices_owed_to_you_overdue"] != "$51,244"
...
def test_overdue_total_of_zero_is_used_as_is_not_fallen_back_from():
    ...
    out = position(data)
    assert out["invoices_owed_to_you_overdue"] == "$0"


def test_a_missing_overdue_total_falls_back_to_the_buckets():
    data = _live()
    data["ar"].pop("overdue_total")
    assert position(data)["invoices_owed_to_you_overdue"] == "$28,498"
...
def test_a_negative_net_carries_its_sign():
    ...
    data["ar"]["overdue_total"] = 0
    out = position(data)
    assert out["net_due_now"] == "-$26,873"
    assert out["net_due_now_amount"] < 0


def test_missing_ap_and_ar_panels_leave_cash_standing_alone():
    out = position({"liquidity": {"cash": 7_742.33}})
    assert out["bills_you_owe_overdue"] == "$0"
    assert out["invoices_owed_to_you_overdue"] == "$0"
    assert out["net_due_now"] == "$7,742"
