...
from app.services.supermemory import merge_chunk_first_hits, merge_memory_and_chunk_hits
...
class MergeChunkFirstHitsTests(unittest.TestCase):
    def test_chunks_lead_and_memories_fill_gaps(self) -> None:
        memories = [
            {
                "id": "m1",
                "memory": "Org structure overview.",
                "metadata": {"fileName": "02_MasterTemplate_OrgStructure", "type": "knowledge_base"},
                "similarity": 0.8,
            }
        ]
        ...
        self.assertEqual(
            (merged[0].get("metadata") or {}).get("fileName"),
            "03_CS_TorrentLaboratories.pdf",
        )
        names = [(h.get("metadata") or {}).get("fileName") for h in merged]
        self.assertIn("02_MasterTemplate_OrgStructure", names)

    def test_same_doc_keeps_chunk_body_not_memory(self) -> None:
        memories = [
            {
                "id": "m1",
                "memory": "Torrent Laboratory brand work.",
