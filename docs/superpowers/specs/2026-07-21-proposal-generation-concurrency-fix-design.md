...
3. **Live generation status exists end-to-end but isn't wired into the buttons that need it.** `proposal_pipeline_checkpoint.py` already tracks `in_progress_phase`/`activity_label` per `rfp_id` in the Supabase-backed `ProposalPipelineCheckpoint`, complete with staleness handling (`_IN_PROGRESS_STALE_SEC = 900`). The frontend already polls this every 4s and even has a mirrored staleness/label helper set (`proposal-pipeline-checkpoint.ts`: `normalizeCheckpointForDisplay`, `inProgressPhaseLabel`, `pipelineServerStillWorkingMessage`, `IN_PROGRESS_STALE_MS = 900_000`) and a derived `serverPipelineActive`/`anyPipelineRunning` value in `ProposalDraftWorkspace.tsx:802-827` that already correctly gates one Generate-triggering button (`ProposalDraftWorkspace.tsx:1886`). But two other spots that control the *empty-state* Generate button and its enclosing view check only the local `isFullProposalRunning` flag, which is `false` for anyone who didn't personally click Generate in that tab — a second user, or the same user after a reload:
   - `ProposalDraftWorkspace.tsx:2180-2181` — the Content-tab view only switches into "generating" display when `outline.sections.some(s => s.content.trim()) || isFullProposalRunning`, so a passive viewer with no content yet and `isFullProposalRunning = false` falls through to the empty-state branch.
   - `ProposalDraftWorkspace.tsx:2357` — that empty-state's own "Generate Proposal" button is `disabled={isFullProposalRunning}` instead of `disabled={anyPipelineRunning}`.
   - `ProposalDraftWorkspace.tsx:1557-1598` (`primaryPipelineLabel`) — only branches on `isFullProposalRunning`, so a passive viewer sees the static label "Generate proposal"/"Continue proposal" instead of the live phase, even on the button that's already correctly disabled.

   This is a wiring gap, not a missing feature — no new backend state, Supabase columns, or staleness logic are needed.
